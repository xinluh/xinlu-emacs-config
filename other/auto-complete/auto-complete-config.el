...
(defvar ac-semantic-candidates-cache)
(defun ac-semantic-candidates (prefix)
  (with-no-warnings
	(setq ac-semantic-candidates-cache
		  (ignore-errors
    ...
    (delete ""            ; semantic sometimes returns an empty string
            (mapcar 'semantic-tag-name ac-semantic-candidates-cache))))

(defun ac-semantic-documentaion (symbol)
...
(ac-define-source semantic-raw
  '((available . (or (require 'semantic-ia nil t)
	...
	;; (candidate-face . ac-yasnippet-candidate-face)
    ;; (selection-face . ac-yasnippet-selection-face)
	(document . ac-semantic-documentaion)
    (candidates . (ac-semantic-candidates ac-prefix))
    (symbol . "s")))
