...
(defcustom dired-details-hide-link-targets nil
            ...
            (if cached-overlays
              ...
              (let ((cache (list (car dir-and-pos)))
                ...
                (goto-char subdir-start)
 				(dired-details-make-current-line-overlay cache t)
...
(defun dired-details-make-current-line-overlay( cache &optional dir-line)
...
(when (and dir-line tramp-dir)
	(overlay-put tramp-dir 'evaporate t)
	(dired-details-hide-overlay tramp-dir))

    (setcdr cache (append (if dir-line
							  (if tramp-dir (list tramp-dir))
							(if ln-target
